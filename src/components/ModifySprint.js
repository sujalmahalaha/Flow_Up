...
const ModifySprint = (props) => {
    ...
    const handleFormSubmit = async (e) => {
      e.preventDefault();
      const response = await fetch(`https://flow-up06.onrender.com/api/sprint/modify-sprint/${params.sprintId}`, {
          method: 'PUT',
          headers: {
    ...
    const fetchSprint = async()=>{
        const response = await fetch(`https://flow-up06.onrender.com/api/sprint/${params.sprintId}`, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
            'auth-token' : localStorage.getItem('token')
    ...
    const fetchTickets = async()=>{
      const response = await fetch(`https://flow-up06.onrender.com/api/project/get-all-tickets/${params.projectId}`, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
            'auth-token' : localStorage.getItem('token')
