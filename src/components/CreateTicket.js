...
const CreateTicket = (props) => {
    ...
    const handleFormSubmit = async (e) => {
      ...
      const response = await fetch("https://flow-up06.onrender.com/api/project/create-ticket", {
          method: 'POST',
          headers: {
    ...
    const fetchProject = async()=>{
        const response = await fetch(`https://flow-up06.onrender.com/api/project/get-project/${projectId}`, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
            'auth-token' : localStorage.getItem('token')
  ...
  return (
    <div className="card">
        <div className="card-header inline">
          <h5 className='heading1'>CREATE TICKET</h5>
        </div>
        ...
        <div className="card-body">  
            <form onSubmit={handleFormSubmit}>
              ...
              <button type="submit" className="newTicketbtn mx-3 my-2"><IoMdCreate className='createIcon' />CREATE</button>
            </form>
        </div>
    </div>
  )
