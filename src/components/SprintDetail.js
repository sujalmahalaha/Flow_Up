...
const SprintDetail = (props) => {
  ...
  const fetchSprint = async()=>{
    const response = await fetch(`https://flow-up06.onrender.com/api/sprint/${sprintId}`, {
      method: 'GET',
      headers: {
    ...
    for (let i = 0; i < json.tickets.length; i++) {
      ...
      const response = await fetch(`https://flow-up06.onrender.com/api/project/get-ticket/${element}`, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
            'auth-token' : localStorage.getItem('token')
  ...
  const handleOnClickInactive = async()=>{
    const response = await fetch(`https://flow-up06.onrender.com/api/sprint/inactive-sprint/${sprintId}`, {
      method: 'POST',
      headers: {
          'Content-Type': 'application/json',
          'auth-token' : localStorage.getItem('token')
      ...
      { sprint.length===0 ?
        ...
        <div className="card">
          <div className="card-header inline">
            ...
            <button disabled={sprint.status==="Inactive"} className="createbtn float-end m-1" onClick={handleOnClickModifySprint}>MODIFY</button>
            
            {sprint.status==="Active"
              ?<button disabled={sprint.status==="Inactive"} className="createbtn float-end m-1" onClick={handleOnClickInactive}>INACTIVE</button>
              :<div></div>
            }
          </div>
